...
def masked_sum(
  value: Tensor, mask: Tensor | None = None, dim=None, keepdim=False
) -> Tensor:
  return (
    value.sum(dim=dim, keepdim=keepdim)
    if mask is None
    else (value * mask).sum(dim=dim, keepdim=keepdim)
  )
...
def masked_var(
  value: Tensor, mask: Tensor | None = None, dim=None, unbiased=False
) -> Tensor:
...
def kl_penalty(
  logprobs: Tensor, reference_logprobs: Tensor, mask: Tensor | None = None
) -> Tensor:
  return masked_mean(logprobs - reference_logprobs, mask)


def discounted_returns(
  rewards: Tensor, gamma: float = 1.0, mask: Tensor | None = None
) -> Tensor:
...
def group_advantages(
  rewards: Tensor, groups: Tensor | Sequence[int] | None = None, eps: float = 1e-8
) -> Tensor:
  if groups is None:
  ...
  for group in groups.unique():
    ...
    output[select] = (values - values.mean()) / values.std(unbiased=False).clamp_min(
      eps
    )
  return output
...
def correctness_gate(
  reward: Tensor, correct: Tensor, incorrect: float = -1.0
) -> Tensor:
