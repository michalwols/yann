...
class PagedTensor:
  ...
  def append(self, element: torch.Tensor):
    ...
    if element.shape != self.element_shape:
      raise ValueError(
        f'Element shape {element.shape} does not match expected shape {self.element_shape}',
      )
  ...
  def __getitem__(self, index):
    ...
    if isinstance(index, int):
      ...
      if not indices:
        return torch.empty(
          (0, *self.element_shape),
          dtype=self.dtype,
          device=self.device,
        )  # Use primary device for empty
  ...
  def __setitem__(self, index, value):
    ...
    if isinstance(index, int):
      ...
      if value.shape != self.element_shape:
        raise ValueError(
          f'Value shape {value.shape} incompatible with element shape {self.element_shape}',
        )
      ...
      if len(indices) != value.shape[0]:
        raise ValueError(
          f'Attempted to assign {value.shape[0]} elements to slice of size {len(indices)}',
        )
      if value.shape[1:] != self.element_shape:
        raise ValueError(
          f'Value tensor shape {value.shape[1:]} incompatible with element shape {self.element_shape}',
        )
  ...
  def __repr__(self):
    return (
      f'PagedTensor(total_elements={self.total_elements}, page_size={self.page_size}, '
