...
class PagedMetricStore:
  ...
  def __init__(
    ...
    self.cpu_paging = (
      cpu_paging and self.device.type == 'cuda'
    )  # Only page if primary device is CUDA
  ...
  def update(
    ...
    **metrics: Dict[str, Union[torch.Tensor, float, int]],
    ...
    for name, value in metrics.items():
      ...
      if (
        self.cpu_paging
        and paged_tensor.pages
        and paged_tensor.current_idx_in_page == paged_tensor.page_size - 1
      ):
  ...
  def to_dict(
    ...
    return {
      name: pt.to_tensor(target_device=target_dev) for name, pt in self.values.items()
    }

  def __repr__(self):
    metric_names = list(self.values.keys())
    return (
      f'PagedMetricStore(steps={len(self)}, metrics={metric_names}, '
