...
class DiffusionParams(yann.train.Trainer.Params):
  ...
  optimizer_config: OptimizerArgs = (
    OptimizerArgs()
  )  # Using nested config for optimizer args
...
def diffusion_step(trainer: Trainer, batch):
  ...
  if vae is None or noise_scheduler is None:
    raise ValueError(
      'VAE model or Noise Scheduler not initialized and passed to Trainer params',
    )
  ...
  if trainer.grad_scaler:  # grad_scaler comes from trainer state based on params.amp
    ...
    if (
      trainer.clip_grad
    ):  # clip_grad instance from trainer init based on params.clip_grad
