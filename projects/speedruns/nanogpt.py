...
class Block(nn.Module):
  def __init__(self, model_dim: int, num_heads: int, layer_idx: int):
    ...
    self.attn = (
      CausalSelfAttention(model_dim, num_heads, layer_idx) if layer_idx != 7 else None
    )
...
class GPT(nn.Module):
  ...
  def forward(
    ...
    sliding_window_num_blocks: Tensor = 0,
  ):
    BLOCK_SIZE = 128
    ...
    def dense_to_ordered(dense_mask: Tensor):
      ...
      return num_blocks[None, None].contiguous(), indices[
        None,
        None,
      ].contiguous()
    ...
    assert (
      len(ve_enc) == self.num_encoder_layers and len(ve_dec) == self.num_decoder_layers
    )
    ...
    x = norm(x)
    logits = (
      self.lm_head(x) if not self.training else lm_head_fp8(x, self.lm_head.weight)
    )
...
def main():
  ...
  adam_params = [
    ...
    dict(params=embed_params, lr=0.6 * k),
